    ...
    const sptr<IRemoteObject> &abilityToken, int32_t abilityId, int32_t continuousTaskId)
{
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    GET_BACK_GROUND_TASK_MANAGER_PROXY_RETURN
    BGTASK_LOGI("abilityName: %{public}s, abilityId: %{public}d, continuousTaskId: %{public}d.",
        abilityName.c_str(), abilityId, continuousTaskId);
    ...
    const BackgroundTaskSubscriber &subscriber)
{
    ...
    return proxy_->SubscribeBackgroundTask(subscriberSptr, subscriber.GetCurrentCallBackType());
}
...
ErrCode BackgroundTaskManager::UnsubscribeBackgroundTask(const BackgroundTaskSubscriber &subscriber)
{
    ...
    return proxy_->UnsubscribeBackgroundTask(subscriberSptr, subscriber.GetCurrentCallBackType());
}
