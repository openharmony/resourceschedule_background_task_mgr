...
export class EnableBgTaskAuthDialog {
  ...
  async createUiExtensionWindow(session: UIExtensionContentSession, stageModel: boolean): Promise<void> {
    try {
      ...
      console.info(`current device type : ${deviceTypeInfo}}`);
      ...
      if (stageModel) {
        ...
        if (moveToCenter) {
          await session.loadContent(EnableBgTaskAuthDialog.EMPTY_PAGE_PATH, this.storage);  
          await session.setWindowBackgroundColor(EnableBgTaskAuthDialog.MASK_COLOR);
        }
