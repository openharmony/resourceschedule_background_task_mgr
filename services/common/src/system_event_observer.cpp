...
void SystemEventObserver::OnReceiveEventContinuousTask(const EventFwk::CommonEventData &eventData)
{
    ...
    int32_t appIndex = want.GetIntParam(AppExecFwk::Constants::APP_INDEX, -1);
    BGTASK_LOGI("get common event action = %{public}s, bundleName = %{public}s, uid = %{public}d, "
        "userId = %{public}d, appIndex = %{public}d,", action.c_str(), bundleName.c_str(), uid, userId, appIndex);
    auto task = [bgContinuousTaskMgr, action, bundleName, uid, userId, appIndex]() {
        bgContinuousTaskMgr->OnBundleInfoChanged(action, bundleName, uid, userId, appIndex);
    };
