...
bool BgContinuousTaskMgr::CheckPermissionForInner(const sptr<ContinuousTaskParamForInner> &taskParam,
    int32_t callingUid)
{
    auto it = g_innerApiReqBgRunningConfig.find(callingUid);
    if (it != g_innerApiReqBgRunningConfig.end()) {
        if (it->second.bgModeId_ == taskParam->bgModeId_) {
            return true;
        }
    }
...
ErrCode BgContinuousTaskMgr::StopBackgroundRunningByTask(const std::shared_ptr<ContinuousTaskRecord> &task)
{
    ...
    auto result = CancelNotification(record);
...
void BgContinuousTaskMgr::HandleRemoveTaskByMode(uint32_t mode)
{
    ...
    while (iter != continuousTaskInfosMap_.end()) {
        ...
        if (record->isFromWebview_ && CommonUtils::CheckExistMode(record->bgModeIds_, mode)) {
            ...
            RefreshTaskRecord();
            auto result = CancelNotification(record);
            BGTASK_LOGI("Cancel notification, uid: %{public}d, result: %{public}d", record->uid_, result);
        } else {
            iter++;
        }
    }
...
bool BgContinuousTaskMgr::CheckLiveViewInfo(std::shared_ptr<ContinuousTaskRecord> record)
{
    if (CommonUtils::CheckExistMode(record->bgModeIds_, BackgroundMode::USB_CONNECTION)) {
        ...
        GetLiveViewAndMediaControllerByUid(record->uid_, liveViewState, mediaControllerState);
    ...
    const std::string &bundleName, int32_t appIndex)
{
    ...
    if (authResult < static_cast<int32_t>(UserAuthResult::DENIED) ||
        authResult >= static_cast<int32_t>(UserAuthResult::END)) {
        BGTASK_LOGE("param auth type is invalid.");
        return;
    }
    int32_t userId = -1;
#ifdef HAS_OS_ACCOUNT_PART
    AccountSA::OsAccountManager::GetOsAccountLocalIdFromUid(bundleUid, userId);
#else // HAS_OS_ACCOUNT_PART
    ...
    if (authRecordIter == bannerNotificationRecord_.end()) {
        BGTASK_LOGE("uid: %{public}d, bundleName: %{public}s, appIndex: %{public}d no have auth record.",
            bundleUid, bundleName.c_str(), appIndex);
        return;
    }
    authRecordIter->second->SetAuthResult(authResult);
    RefreshAuthRecord();
    // 触发回调
    auto callbackIter = expiredCallbackMap_.find(key);
    if (callbackIter != expiredCallbackMap_.end()) {
...
ErrCode BgContinuousTaskMgr::RemoveAuthRecord(const sptr<ContinuousTaskParam> &taskParam)
{
    if (!isSysReady_.load()) {
        BGTASK_LOGW("manager is not ready");
        return ERR_BGTASK_SYS_NOT_READY;
    ...
    pid_t callingPid = IPCSkeleton::GetCallingPid();
    std::string bundleName = BundleManagerHelper::GetInstance()->GetClientBundleName(callingUid);
    std::shared_ptr<ContinuousTaskRecord> record = std::make_shared<ContinuousTaskRecord>(bundleName,
        "", callingUid, callingPid, taskParam->bgModeId_, true, taskParam->bgModeIds_);
    InitRecordParam(record, taskParam, userId);
    ErrCode ret = ERR_OK;
    handler_->PostSyncTask([this, record, &ret]() {
        ret = this->RemoveAuthRecordInner(record);
        }, AppExecFwk::EventQueue::Priority::HIGH);
    return ret;
}
...
ErrCode BgContinuousTaskMgr::RemoveAuthRecordInner(const std::shared_ptr<ContinuousTaskRecord> record)
{
    ...
    if (authRecordIter != bannerNotificationRecord_.end()) {
        BGTASK_LOGI("uid: %{public}d, bundleName: %{public}s, appIndex: %{public}d remove auth record.",
            record->uid_, record->bundleName_.c_str(), record->appIndex_);
        bannerNotificationRecord_.erase(key);
        RefreshAuthRecord();
    }
    auto callbackIter = expiredCallbackMap_.find(key);
...
void BgContinuousTaskMgr::GetLiveViewAndMediaControllerByUid(int32_t uid, bool &liveViewState,
    bool &mediaControllerState)
{
    liveViewState = BgtaskDataMgr::GetInstance()->CheckLiveViewInfoByUid(uid);
...
void BgContinuousTaskMgr::ReportTaskAdjustEventByUid(int32_t uid)
{
    ...
    bool liveViewState = false;
    bool mediaControllerState = false;
    GetLiveViewAndMediaControllerByUid(uid, liveViewState, mediaControllerState);
    ...
    ReportTaskAdjustEvent(pidModeMap, payload);
}
    ...
    ContinuousTaskEventTriggerType changeEventType)
{
    ...
    payload["mediaControllerState"] = mediaControllerState;
    std::map<pid_t, std::set<uint32_t>> pidModeMap;
    std::string taskInfoMapKey = std::to_string(record->uid_) + SEPARATOR + record->abilityName_ +
        SEPARATOR + std::to_string(record->abilityId_);
    if (record->isByRequestObject_) {
        taskInfoMapKey += SEPARATOR + std::to_string(record->continuousTaskId_);
    }
    for (const auto &task : continuousTaskInfosMap_) {
...
void BgContinuousTaskMgr::ReportTaskAdjustEvent(const std::map<pid_t, std::set<uint32_t>> &pidModeMap,
    nlohmann::json &payload)
{
    ...
    BGTASK_LOGD("ReportTaskAdjustEvent info: %{public}s",
...
void BgContinuousTaskMgr::HandleReportAdjustEvent(int32_t uid)
{
    ...
    auto task = [weak = weak_from_this(), uid] {
        auto self = weak.lock();
        if (self) {
