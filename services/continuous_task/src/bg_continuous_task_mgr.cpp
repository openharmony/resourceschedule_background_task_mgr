...
void BgContinuousTaskMgr::ReportTaskRequiredStateChanged(int32_t uid, int32_t pid, uint32_t taskType)
{
    BGTASK_LOGD("ReportTaskRequiredStateChanged begin uid: %{public}d, pid: %{public}d, taskType: %{public}d",
        uid, pid, taskType);
    if (!isSysReady_.load()) {
        BGTASK_LOGW("manager is not ready");
        return;
