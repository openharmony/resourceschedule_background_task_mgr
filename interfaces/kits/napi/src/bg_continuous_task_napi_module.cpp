...
void UpdateBackgroundRunningByRequestExecuteCB(napi_env env, void *data)
{
    ...
    const std::shared_ptr<AppExecFwk::AbilityInfo> info = asyncCallbackInfo->abilityContext->GetAbilityInfo();
    ContinuousTaskParam taskParam = ContinuousTaskParam(true, backgroundTaskModes[0],
        asyncCallbackInfo->request->GetWantAgent(), info->name, asyncCallbackInfo->abilityContext->GetToken(),
        "", true, backgroundTaskModes, asyncCallbackInfo->abilityContext->GetAbilityRecordId());
...
{
    std::vector<uint32_t> backgroundTaskModes = asyncCallbackInfo->request->GetBackgroundTaskModes();
    std::vector<uint32_t> backgroundTaskSubmodes = asyncCallbackInfo->request->GetBackgroundTaskSubmodes();
    if (backgroundTaskModes.size() == 0 || backgroundTaskSubmodes.size() == 0) {
    ...
    if (backgroundTaskModes.size() != backgroundTaskSubmodes.size()) {
    ...
    if (static_cast<uint32_t>(specialModeSize) > MAX_TASK_NUMS) {
    ...
    if (static_cast<uint32_t>(specialModeSize) == MAX_TASK_NUMS && backgroundTaskModes.size() > MAX_TASK_NUMS) {
    ...
    for (uint32_t index = 0; index < backgroundTaskSubmodes.size(); index++) {
        ...
        if (subMode >= BackgroundTaskSubmode::END ||
            subMode < BackgroundTaskSubmode::SUBMODE_CAR_KEY_NORMAL_NOTIFICATION) {
        ...
        if (subMode == BackgroundTaskSubmode::SUBMODE_NORMAL_NOTIFICATION) {
            if (!BackgroundTaskMode::IsModeTypeMatching(backgroundTaskModes[index])) {
            ...
            if (BackgroundTaskMode::GetSubModeTypeMatching(subMode) != backgroundTaskModes[index]) {
                Common::HandleErrCode(env, ERR_BGTASK_CONTINUOUS_MODE_OR_SUBMODE_TYPE_MISMATCH, true);
                asyncCallbackInfo->errCode = ERR_BGTASK_CONTINUOUS_MODE_OR_SUBMODE_TYPE_MISMATCH;
                return false;
            }
...
bool IsModeSupportedExecuteCB(napi_env env, AsyncCallbackInfo *asyncCallbackInfo)
{
    ...
    uint32_t oldMode = 0;
    for (auto mode : asyncCallbackInfo->request->GetBackgroundTaskModes()) {
        oldMode = BackgroundTaskMode::GetV9BackgroundModeByMode(mode);
        backgroundTaskModes.push_back(oldMode);
    }
    ContinuousTaskParam taskParam = ContinuousTaskParam(true, -1,
        nullptr, "", nullptr, "", true, backgroundTaskModes, -1);
...
bool CheckSpecialModeSupported(napi_env env, AsyncCallbackInfo *asyncCallbackInfo)
{
    std::vector<uint32_t> backgroundTaskModes = asyncCallbackInfo->request->GetBackgroundTaskModes();
    if (backgroundTaskModes.size() == 0) {
        Common::HandleErrCode(env, ERR_BGTASK_CONTINUOUS_MODE_OR_SUBMODE_IS_EMPTY, true);
...
napi_value IsModeSupported(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok ||
        !Common::GetBackgroundTaskModesFromArray(env, backgroundTaskModes, asyncCallbackInfo->request)) {
        asyncCallbackInfo->errCode = ERR_BGTASK_CONTINUOUS_REQUEST_NULL_OR_TYPE;
        BGTASK_LOGE("failed to check params");
        return WrapVoidToJS(env);
    }
    if (!CheckSpecialModeSupported(env, asyncCallbackInfo)) {
...
void RequestAuthFromUserExecuteCB(napi_env env, void *data)
{
    AsyncCallbackInfo *asyncCallbackInfo = static_cast<AsyncCallbackInfo *>(data);
    if (asyncCallbackInfo == nullptr) {
        BGTASK_LOGE("input params error");
    ...
    ContinuousTaskParam taskParam = ContinuousTaskParam(true, backgroundTaskModes[0],
        nullptr, "", nullptr, "", true, backgroundTaskModes, -1);
    taskParam.isByRequestObject_ = true;
    asyncCallbackInfo->errCode = BackgroundTaskMgrHelper::RequestAuthFromUser(taskParam);
}

void RequestAuthFromUserPromiseCompletedCB(napi_env env, napi_status status, void *data)
{
    AsyncCallbackInfo *asyncCallbackInfo = static_cast<AsyncCallbackInfo *>(data);
    std::unique_ptr<AsyncCallbackInfo> callbackPtr {asyncCallbackInfo};
    napi_value result {nullptr};
    if (asyncCallbackInfo->errCode == ERR_OK) {
        int32_t resu = 2;
        napi_create_int32(env, resu, &result);
        NAPI_CALL_RETURN_VOID(env, napi_resolve_deferred(env, asyncCallbackInfo->deferred, result));
    } else {
        std::string errMsg = Common::FindErrMsg(env, asyncCallbackInfo->errCode);
        int32_t errCodeInfo = Common::FindErrCode(env, asyncCallbackInfo->errCode);
...
napi_value RequestAuthFromUserPromise(napi_env env, AsyncCallbackInfo *asyncCallbackInfo)
{
    if (asyncCallbackInfo == nullptr) {
        BGTASK_LOGE("param is nullptr");
        return nullptr;
    ...
    NAPI_CALL(env, napi_create_async_work(env,
        nullptr,
        resourceName,
        RequestAuthFromUserExecuteCB,
        RequestAuthFromUserPromiseCompletedCB,
        static_cast<void *>(asyncCallbackInfo),
        &asyncCallbackInfo->asyncWork));
    NAPI_CALL(env, napi_queue_async_work(env, asyncCallbackInfo->asyncWork));
    return promise;
...
napi_value RequestAuthFromUser(napi_env env, napi_callback_info info)
{
    ...
    napi_value ret = RequestAuthFromUserPromise(env, asyncCallbackInfo);
    callbackPtr.release();
    if (ret == nullptr) {
        BGTASK_LOGE("ret is nullpter");
        if (asyncCallbackInfo != nullptr) {
...
void CheckSpecialScenarioAuthExecuteCB(napi_env env, void *data)
{
    AsyncCallbackInfo *asyncCallbackInfo = static_cast<AsyncCallbackInfo *>(data);
    if (asyncCallbackInfo == nullptr) {
        BGTASK_LOGE("input params error");
...
void CheckSpecialScenarioAuthPromiseCompletedCB(napi_env env, napi_status status, void *data)
{
    ...
    napi_value result {nullptr};
    if (asyncCallbackInfo->errCode == ERR_OK) {
        napi_create_int32(env, asyncCallbackInfo->authResult, &result);
        NAPI_CALL_RETURN_VOID(env, napi_resolve_deferred(env, asyncCallbackInfo->deferred, result));
    } else {
        std::string errMsg = Common::FindErrMsg(env, asyncCallbackInfo->errCode);
        int32_t errCodeInfo = Common::FindErrCode(env, asyncCallbackInfo->errCode);
...
napi_value CheckSpecialScenarioAuthPromise(napi_env env, AsyncCallbackInfo *asyncCallbackInfo)
{
    ...
    NAPI_CALL(env, napi_create_async_work(env,
        nullptr,
        resourceName,
        CheckSpecialScenarioAuthExecuteCB,
        CheckSpecialScenarioAuthPromiseCompletedCB,
        static_cast<void *>(asyncCallbackInfo),
        &asyncCallbackInfo->asyncWork));
    NAPI_CALL(env, napi_queue_async_work(env, asyncCallbackInfo->asyncWork));
    return promise;
...
napi_value CheckSpecialScenarioAuth(napi_env env, napi_callback_info info)
{
    ...
    napi_value jsThis;
    napi_get_cb_info(env, info, nullptr, nullptr, &jsThis, nullptr);
    napi_value ret = CheckSpecialScenarioAuthPromise(env, asyncCallbackInfo);
    callbackPtr.release();
    if (ret == nullptr) {
        BGTASK_LOGE("ret is nullpter");
        if (asyncCallbackInfo != nullptr) {
